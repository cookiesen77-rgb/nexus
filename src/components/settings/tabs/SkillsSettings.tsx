...
export function SkillsSettings({
  ...
  const loadSkillsFromPath = async (skillsPath: string) => {
    setLoading(true);
    try {
      // Get all skills directories (nexuswork and claude)
      ...
      if (dirsData.directories) {
        for (const dir of dirsData.directories as {
          ...
          // Only load from user directory (claude), skip app directory (nexuswork)
              ...
              for (const folder of filesData.files) {
                if (folder.isDir) {
                  ...
                  allSkills.push({
                    ...
                    name: skillName,
                    source: dir.name as 'claude' | 'nexuswork',
                    path: folder.path,
