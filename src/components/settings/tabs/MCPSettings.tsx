...
import { getNexusWorkMcpPath } from '@/shared/lib/paths';
...
export function MCPSettings({ settings, onSettingsChange }: SettingsTabProps) {
  ...
  useEffect(() => {
    getNexusWorkMcpPath().then(setDefaultMcpPath);
  }, []);
    ...
    async function loadMCPConfig() {
      ...
      try {
        ...
        for (const configInfo of result.configs as {
          ...
          } else if (configInfo.name === 'nexuswork') {
          ...
          for (const [id, serverConfig] of Object.entries(configInfo.servers)) {
            ...
            serverList.push({
              ...
              autoExecute: true,
              source: configInfo.name as 'nexuswork' | 'claude',
            });
          }
        }
    ...
    try {
      ...
      for (const [name, config] of Object.entries(mcpServers)) {
        ...
        const existingIndex = newServers.findIndex(
          (s) => s.name === name && s.source === 'nexuswork'
        );
  ...
  const handleSaveConfigDialog = () => {
    ...
    if (configDialog.mode === 'edit' && configDialog.editServerId) {
      ...
      const fullId = `nexuswork-${configDialog.serverName}`;
      if (
        newServers.some(
